...
The output of `lsp_thumbprint()`, `landcover_comp`,  has a new class `lsp`.
...
Each ecoregion is also related to an unique value in the `id` column.
...
Now, we need to provide this dataset and its identificator column to the `window` argument of `lsp_thumbprint()`.
