...
This time, we set the `window` argument to `200`, which means that each local landscape will consist of 200 by 200 cell.
In this example, each cell has a resolution of 300 by 300 meters, therefore a local landscape will be 60000 by 60000 meters.
...
```{r, echo=FALSE}
plot(ecoregions["id"], main = NULL)
```


```{r}
landcover_coma_e = lsp_thumbprint(landcover, type = "coma", window = ecoregions["id"])
landcover_coma_e
```

<!-- ADD INFO ABOUT STARS PROXY -->
