...
In the first example, we will divide the whole area into many regular local landscapes, and find the way to cluster them based on their patterns.
...
Next, we need to calculate a distance between each local landscape.
...
This allows it to be used by many existing R functions for clustering, that expect a distance matrix (`dist`) as an input.
It includes different approaches to hierarchical clustering (`hclust()`, `cluster::agnes()`, `cluster::diana()`), fuzzy clustering (`funny()`).
More information about clustering techniques in R can be find in [CRAN Task View: Cluster Analysis & Finite Mixture Models](https://cran.r-project.org/web/views/Cluster.html).
...
Based on the obtained dendrogram, we are able to divide our local landscapes into a specified number of groups using `cutree()`.
In this example, we will use six classes (`k = 6`) to make a result fairly staightforward to understand.

```{r}
clusters = cutree(landcover_hclust, k = 6)
```

The **motif** package allows to represent the clustering results as either `stars` or `sf` object. 
This can be done with the `lsp_add_clusters()` function.

### `stars`

By the default, it creates a `stars` object with a new attribute `clust`:
...
We can also calculate the quality of the clustering with the `lsp_add_quality()` function, which requires an output of `lsp_add_clusters` as the first argument and an output of `lsp_to_dist()` as the second one.
It adds three new attributes: `inhomogeneity`, `isolation`, and `quality`.
...
This value is between 0 and 1, where small value indicates that all objects in the cluster represent consistent patterns so the cluster is pattern-homogeneous. 
...
This value is between 0 and 1, where large value indicates that the cluster stands out from its surroundings.
...
To create an `sf` object with a new variable `clust`, we should set `output = "sf"`.
The rest of calculations are alost identical to the `stars` examples above.

```{r}
landcover_grid_sf = lsp_add_clusters(landcover_cove, clusters, output = "sf")
...
plot(landcover_grid_stars2["clust"], col = safe_pal)

landcover_grid_starsq2 = lsp_add_quality(landcover_grid_stars2, landcover_dist2)

plot(landcover_grid_starsq2["inhomogeneity"])
plot(landcover_grid_starsq2["isolation"])
plot(landcover_grid_starsq2["quality"])
```
...
```{r}
# sf ----------------------------------------------------------------------
landcover_grid_sf2 = lsp_add_clusters(landcover_cove2, clusters2, output = "sf")

# landcover_grid_sf$clust = as.factor(landcover_grid_sf$clust)
plot(landcover_grid_sf2["clust"], pal = safe_pal)
# mapview::mapview(landcover_grid_sf["clust"])

landcover_grid_sfq2 = lsp_add_quality(landcover_grid_sf2, landcover_dist2)
plot(landcover_grid_sfq2["inhomogeneity"])
plot(landcover_grid_sfq2["isolation"])
plot(landcover_grid_sfq2["quality"])
```
