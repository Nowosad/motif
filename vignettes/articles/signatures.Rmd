...
Main element of the pattern-based spatial analysis is to numericaly describe spatial patterns.
This numerical describction is called a signature.
This vignette shows how to derive spatial signatures using the `lsp_thumbprint()` function on example datasets.
...
For this vignette, we will use several spatial objects.
...
It has fourteen land form categories (plus surface water) for the same area.
...
It is an artificial datasets representing numerical weights for each cell for the same area as the datasets above.
...
```{r}
coma_output = lsp_thumbprint(landcover, type = "coma", window = 100)
coma_output
```
...
```{r}
cove_output = lsp_thumbprint(landcover, type = "cove", window = 100)
cove_output
```
...
The first one is a categorical raster data, while the second one is a continuous raster data containing weights for each cell.

```{r}
wecoma_output = lsp_thumbprint(c(landcover, random_ndvi), 
...
```{r}
wecove_output = lsp_thumbprint(c(landcover, random_ndvi),
                               type = "wecove", window = 100)
wecove_output
```

Learn more about these representation at https://nowosad.github.io/comat/articles/wecoma.html.
...
```{r}
incoma_output = lsp_thumbprint(c(landcover, landform),
...
```{r}
incove_output = lsp_thumbprint(c(landcover, landform),
                               type = "incove", window = 100)
```

<!-- Learn more about these representation at https://nowosad.github.io/comat/articles/incoma.html -->
...
```{r}
composition_output = lsp_thumbprint(landcover, 
...
```{r}
composition_output2 = lsp_thumbprint(landcover, 
                                    type = "composition", window = 100,
                                    normalization = "none")
...
The **motif** package also allows to calculate signature based on a user-defined function.
This function should accept only one argument, which is a list containing one or more matrices.
For example `my_fun()` below counts how many non-NA cells exist in each local local landscapes.
...
Importantly, we need to set `normalization = "none"` to prevent normalization of the output.

```{r}
my_fun_output = lsp_thumbprint(landcover, 
